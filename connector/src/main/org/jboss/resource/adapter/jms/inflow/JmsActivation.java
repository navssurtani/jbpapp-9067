...
public class JmsActivation implements ExceptionListener
{
   ...
   protected void setupConnection(Context ctx) throws Exception
   {
      ...
      if (spec.getPreferFactoryRef() && isFactoryRefAvailable())
      {
    	  connection = setupGenericConnection(ctx, user, pass, clientID);
      }
      else
      {
    	  if (isTopic)
   ...
   /**
    * Determines if the Combined factory is available on the JMS provider
    */
   private Boolean isFactoryRefAvailable()
   {
	   try 
	   {
		   adapter.getFactoryRef();
		   return true;
	   }
    ...
    * @throws Exception for any error
    */
   protected Connection setupGenericConnection(Context ctx, String user, String pass, String clientID) throws Exception
   {
      String qenericFactoryRef = adapter.getFactoryRef();
      log.debug("Attempting to lookup combined connection factory " + qenericFactoryRef);
      ConnectionFactory gcf = (ConnectionFactory) Util.lookup(ctx, qenericFactoryRef, ConnectionFactory.class);
      log.debug("Got generic connection factory " + gcf + " from " + qenericFactoryRef);
      log.debug("Attempting to create generic connection with user " + user);
      Connection result;
